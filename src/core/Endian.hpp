...
/*     constexpr auto reverse_bytes(const Sha256Hash& hash) -> Sha256Hash {
        Sha256Hash result{};
        std::ranges::transform(hash, result.begin(), [](uint32_t x) { return
   std::byteswap(x); }); std::ranges::reverse(result); return result;
    } */
...
constexpr auto reverse_bytes(const std::array<T, N>& value) noexcept
    -> std::array<T, N> {
    std::array<T, N> result;
    std::ranges::transform(value, result.begin(), [](T x) -> T {
...
template<std::unsigned_integral T>
constexpr auto
from_le_bytes(const std::array<uint8_t, sizeof(T)>& bytes) noexcept -> T {
    T value = 0;
    for (std::size_t i = 0; i < sizeof(T); ++i) {
...
template<std::unsigned_integral T>
constexpr auto
from_be_bytes(const std::array<uint8_t, sizeof(T)>& bytes) noexcept -> T {
    T value = 0;
    for (std::size_t i = 0; i < sizeof(T); ++i) {
...
template<std::unsigned_integral T, std::size_t N>
constexpr auto to_le_bytes(const std::array<T, N>& value) noexcept
...
template<std::unsigned_integral T, std::size_t N>
constexpr auto to_be_bytes(const std::array<T, N>& value) noexcept
    -> std::array<uint8_t, sizeof(T) * N> {
    std::array<uint8_t, sizeof(T) * N> bytes{};
...
template<std::unsigned_integral T, std::size_t N>
constexpr auto
from_le_bytes(const std::array<uint8_t, sizeof(T) * N>& bytes) noexcept
    -> std::array<T, N> {
    std::array<T, N> value{};
    for (std::size_t i = 0; i < N; ++i) {
...
template<std::unsigned_integral T, std::size_t N>
constexpr auto
from_be_bytes(const std::array<uint8_t, sizeof(T) * N>& bytes) noexcept
    -> std::array<T, N> {
    std::array<T, N> value{};
    for (std::size_t i = 0; i < N; ++i) {
        std::array<uint8_t, sizeof(T)> element_bytes{};
