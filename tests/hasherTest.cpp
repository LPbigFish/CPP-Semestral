...
TEST(OpensslHasherTest, GenesisBlockDoubleHash) {
    ...
    Sha256Hash result = OpensslHasher::double_hash_bytes(header);
...
TEST(OpensslHasherTest, GenesisBlockStreamingMatchesOneShot) {
    ...
    Sha256Hash oneshot_result = OpensslHasher::hash_bytes(header);
...
TEST(OpensslHasherTest, ResetAllowsReuse) {
    ...
    OpensslHasher fresh;
    Sha256Hash expected = OpensslHasher::hash_bytes(header);
    ASSERT_EQ(first, expected);
}

TEST(OwnHasherTest, GenesisBlockDoubleHash) {
    const auto header = genesis_header_bytes();
    std::println("Header: {}", core::bytes_to_hex(header));
    OwnHasher hasher;
    Sha256Hash result = OwnHasher::double_hash_bytes(header);
    Sha256Hash expected = Sha256Hash::from_hex(
        "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
    );
    ASSERT_EQ(result, expected);
